...
// See: [http://www.x-io.co.uk/node/8#open_source_ahrs_and_imu_algorithms](http://www.x-io.co.uk/node/8#open_source_ahrs_and_imu_algorithms)
//
// Date			Author			Notes
...
// 03/12/2025	Bill Gu		    Integrated into gy9163-AHRS
//
//=====================================================================================================
...
MadgwickAHRS* create_madgwick_ahrs(MA_PRECISION sample_rate, MA_PRECISION beta){
    ...
    if (workspace == NULL) return NULL; // Check malloc result

	workspace->sample_rate = sample_rate;
    workspace->beta = beta; // Use provided beta
	// Initial quaternion of sensor frame relative to auxiliary frame (identity quaternion)
	workspace->q0 = 1.0f;
	workspace->q1 = 0.0f;
	workspace->q2 = 0.0f;
	workspace->q3 = 0.0f;
    // Initialize Euler angles too
    workspace->roll = 0.0f;
    workspace->pitch = 0.0f;
    workspace->yaw = 0.0f;
	return workspace;
}
...
void madgwick_ahrs_update_imu(MadgwickAHRS* workspace, MA_PRECISION gx, MA_PRECISION gy, MA_PRECISION gz, MA_PRECISION ax, MA_PRECISION ay, MA_PRECISION az)
{
    ...
    MA_PRECISION sample_freq = workspace->sample_rate; // Local copy
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		// Apply feedback step
		qDot1 -= workspace->beta * s0;
		qDot2 -= workspace->beta * s1;
		qDot3 -= workspace->beta * s2;
		qDot4 -= workspace->beta * s3;
	}

	// Integrate rate of change of quaternion to yield quaternion
	workspace->q0 += qDot1 * (1.0f / sample_freq);
	workspace->q1 += qDot2 * (1.0f / sample_freq);
	workspace->q2 += qDot3 * (1.0f / sample_freq);
	workspace->q3 += qDot4 * (1.0f / sample_freq);
...
void madgwick_ahrs_update(MadgwickAHRS* workspace, MA_PRECISION gx, MA_PRECISION gy, MA_PRECISION gz, MA_PRECISION ax, MA_PRECISION ay, MA_PRECISION az, MA_PRECISION mx, MA_PRECISION my, MA_PRECISION mz)
{
	...
	if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
		...
		_2bx = SQRT(hx * hx + hy * hy); // magnitude of horizontal component of magnetic field
		_2bz = -_2q0mx * workspace->q2 + _2q0my * workspace->q1 + mz * q0q0 + _2q1mx * workspace->q3 - mz * q1q1 + _2q2 * my * workspace->q3 - mz * q2q2 + mz * q3q3; // vertical component of magnetic field
		_4bx = 2.0f * _2bx;
		_4bz = 2.0f * _2bz;
		...
		s1 = _2q3 * (2.0f * q1q3 - _2q0q2 - ax) + _2q0 * (2.0f * q0q1 + _2q2q3 - ay) - 4.0f * workspace->q1 * (1.0f - 2.0f * q1q1 - 2.0f * q2q2 - az) + _2bz * workspace->q3 * (_2bx * (0.5f - q2q2 - q3q3) + _2bz * (q1q3 - q0q2) - mx) + (_2bx * workspace->q2 + _2bz * workspace->q0) * (_2bx * (q1q2 - q0q3) + _2bz * (q0q1 + q2q3) - my) + (_2bx * workspace->q3 - _4bz * workspace->q1) * (_2bx * (q0q2 + q1q3) + _2bz * (0.5f - q1q1 - q2q2) - mz);
		s2 = -_2q0 * (2.0f * q1q3 - _2q0q2 - ax) + _2q3 * (2.0f * q0q1 + _2q2q3 - ay) - 4.0f * workspace->q2 * (1.0f - 2.0f * q1q1 - 2.0f * q2q2 - az) + (-_4bx * workspace->q2 - _2bz * workspace->q0) * (_2bx * (0.5f - q2q2 - q3q3) + _2bz * (q1q3 - q0q2) - mx) + (_2bx * workspace->q1 + _2bz * workspace->q3) * (_2bx * (q1q2 - q0q3) + _2bz * (q0q1 + q2q3) - my) + (_2bx * workspace->q0 - _4bz * workspace->q2) * (_2bx * (q0q2 + q1q3) + _2bz * (0.5f - q1q1 - q2q2) - mz);
